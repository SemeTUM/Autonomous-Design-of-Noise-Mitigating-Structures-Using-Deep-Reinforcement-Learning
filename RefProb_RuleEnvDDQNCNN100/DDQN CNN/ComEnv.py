...
def select_and_modify_pixels(matrix, n):
    ...
    # Track the count of states changed to 2
    count_changed_to_2 = 0

    # Perform the random modifications
    for _ in range(n):
        # Choose a random direction (left, right, up, down)
        direction = random.choice(['left', 'right', 'up', 'down'])

        # Update the position based on the chosen direction
        if direction == 'left' and current_col > 0:
            current_col -= 1
    ...
    while count_changed_to_2 < n:
        # Perform a random walk
        direction = random.choice(['left', 'right', 'up', 'down'])

        # Update the position based on the chosen direction, handling edges
        if direction == 'left' and current_col > 0:
            current_col -= 1
        elif direction == 'right' and current_col < cols - 1:
            current_col += 1
            ...
            # If an edge is encountered, choose a direction convenient for connection
            direction = random.choice(['left', 'right', 'up', 'down'])
